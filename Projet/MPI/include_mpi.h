...
//fusion to array
 ...
 int p,test=0,*mm = malloc(sizeof(int)*(*size));
   ...
   for(int j=1;j<= *size;j=j+4){  faux=0;
...
void allocate(int *tab,int * buf,int rang,int size,int runs){

  if(rang == 0){

        tab = (int*)malloc((sizeof(int)*runs*4+1)/*size*/);
        buf= (int*)malloc(sizeof(int)*runs*4*(size-1));
  
 }
 else tab = (int*)malloc(sizeof(int)*runs*4+1);

 }

// free memory

void free_array(int *tab,int *buf,int rang){
